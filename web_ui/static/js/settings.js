...
/* Everything below runs inside `initSettings`, called once when the controls
 * are actually on the page -- either because this is the Settings page, or
 * because the panel has just fetched them into the chat.
 *
 * Once, and only once. Several of these are delegated `document` listeners, so
 * running it a second time would double every click. The panel therefore keeps
 * the markup after it is closed and simply hides it, rather than tearing it
 * down and rebuilding.
 */
...
function initSettings() {
    ...
    (function () {
        ...
        // inspecting, or viewing source -- and no way in from this page at all.
        // Reading or changing it means turning child mode off, which already
        // costs the parent password. One lock, one door.
        (function () {
            ...
            saveBtn.addEventListener('click', async () => {
                const r = await post('/api/child/note/save', { note: box.value });
                if (r.ok) {
                    say(r.saved ? 'Saved. The AI follows this from its next reply.'
                                : 'Cleared.');
...
window.__initSettings = initSettings;
// The Settings page has the controls in it from the start; the panel calls this
// itself after it has fetched them.

...
register(Tool(
    ...
    description=(
        ...
        "**Anything that stays running belongs in `preview`, not here** -- a dev "
        "server, an API, a watcher. Backgrounded with `&` here it survives this "
        "call and then nothing can stop it: not Play/Stop, not closing the "
        "project, not deleting it. The user has no terminal, so a server started "
        "this way holds its port until the computer is restarted, and the next "
...
register(Tool(
    ...
    description=(
        ...
        "**A project needing two servers is one command, not two calls.** An API "
        "and a front end is the ordinary shape of web work: put both in `command` "
        "with `&` between them -- `uvicorn api:app --port 8000 & npm run dev` -- "
        "and give `url` for the one the user looks at. They start together and "
        "they stop together, because everything in that command is owned by this "
        "tool. Starting the API with `bash` instead leaves something running that "
        "nothing in this app can stop.\n"
        "There is ONE preview per project: starting again stops the old one and "
        "reuses the same window, so the user never accumulates stale tabs. Call "
...
register(Tool(
    ...
    description=(
        ...
        "what can draw on this computer and what each one charges -- the list "
        "is found by asking, not fixed, so it is right for whatever keys this "
        "machine has today. Not every computer can draw, and one that could "
        "not this morning may have had a key added at lunchtime, so ask rather "
        ...
        "costs and wait for a yes; after that, get on with it. Choose the "
        "model yourself from the prices in that list -- a background does not "
        "need the dear one. Only put the choice to them when the list says "
        "nothing there is certain to work.\n"
